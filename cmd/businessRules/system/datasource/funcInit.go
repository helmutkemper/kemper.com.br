...
func (e *RefList) Init(name Name) (err error) {
	...
	case KMongoDB:
		...
		// menu
		path, err = util.FileFindInTree("menu.mongodb.so")
		if err != nil {
			util.TraceToLog()
			return
		}

		err = e.installMenuByPlugin(path)
		if err != nil {
			util.TraceToLog()
			return
		}
		...
		// languages
		path, err = util.FileFindInTree("languages.mongodb.so")
		if err != nil {
			util.TraceToLog()
			return
		}

		err = e.installLanguageByPlugin(path)
		if err != nil {
			util.TraceToLog()
			return
		}
	...
	case KSQLite:
		path, err = util.FileFindInTree("menu.sqlite.so")
		if err != nil {
			util.TraceToLog()
			return
		}
		...
		path, err = util.FileFindInTree("languages.sqlite.so")
		if err != nil {
			util.TraceToLog()
			return
		}
